...
def create_custom_optimizer(
    ...
    if finetuning_args.use_dynamic_rank:
        # Create layer-wise optimizers for dynamic rank training
        optimizers = {}
        
        # Get knowledge scores for each layer
        knowledge_scores = _get_layer_knowledge_scores(model, finetuning_args)
        
        # Create optimizers with different learning rates based on knowledge scores
        for name, param in model.named_parameters():
            if param.requires_grad:
                layer_name = _get_layer_name(name)
                if layer_name in knowledge_scores:
                    # Scale learning rate based on knowledge importance
                    lr_scale = 1.0 + knowledge_scores[layer_name]
                    layer_lr = training_args.learning_rate * lr_scale
                    
                    # Create optimizer for this layer
                    optimizers[param] = torch.optim.AdamW(
                        [param],
                        lr=layer_lr,
                        weight_decay=training_args.weight_decay
                    )
        
        # Return dummy optimizer that manages the layer-wise optimizers
...
class KnowledgeAwareTrainer(Trainer):
    """Custom trainer with knowledge-aware features."""
    
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.teacher_model = None
        if self.args.finetuning_args.use_dynamic_rank:
            # Initialize teacher model for knowledge distillation
            teacher_model_args = ModelArguments.copyfrom(self.args.model_args)
            ...
            self.teacher_model = load_model(
                tokenizer, 
                teacher_model_args, 
                teacher_finetuning_args, 
                is_trainable=False
            )
            self.teacher_model.eval()
            logger.info_rank0("Initialized teacher model for knowledge distillation")
    
    def compute_loss(self, model, inputs, return_outputs=False):
        outputs = model(**inputs)
        loss = outputs.loss
        
        # Add knowledge distillation loss if using dynamic rank
        if self.args.finetuning_args.use_dynamic_rank:
            kd_loss = self._compute_knowledge_distillation_loss(model, inputs)
            loss = loss + self.args.finetuning_args.kd_weight * kd_loss
            
        return (loss, outputs) if return_outputs else loss
    
    def _compute_knowledge_distillation_loss(self, model, inputs):
        """Compute knowledge distillation loss to preserve domain knowledge."""
        ...
        temperature = self.args.finetuning_args.kd_temperature
    ...
    def create_optimizer(self):
        """Create knowledge-aware optimizer."""
        if self.args.finetuning_args.use_dynamic_rank:
    ...
    def _maybe_log_save_evaluate(self, tr_loss, model, trial, epoch, ignore_keys_for_eval):
        """Add knowledge-aware metrics to logging."""
        if self.args.finetuning_args.use_dynamic_rank:
            # Log rank distribution
            ranks = {}
            for name, config in model.peft_config.items():
                if hasattr(config, 'rank_pattern'):
                    ranks[name] = config.rank_pattern
                else:
                    ranks[name] = config.r
            
            # Calculate statistics
            all_ranks = [r for rank_dict in ranks.values() for r in rank_dict.values()]
            metrics = {
                "knowledge_metrics/avg_rank": np.mean(all_ranks),
                "knowledge_metrics/max_rank": max(all_ranks),
                "knowledge_metrics/min_rank": min(all_ranks),
                "knowledge_metrics/rank_distribution": ranks
            }
            
            # Log knowledge scores by layer position
            layer_scores = {}
            for name, module in model.named_modules():
                if isinstance(module, torch.nn.Linear):
                    ...
                    if layer_idx is not None:
                        layer_scores[f"layer_{layer_idx}"] = {
                            "rank": ranks.get(name, {}).get(name, 0),
                            "position": layer_idx / model.config.num_hidden_layers
                        }
            
            metrics["knowledge_metrics/layer_scores"] = layer_scores
            self.log(metrics)
